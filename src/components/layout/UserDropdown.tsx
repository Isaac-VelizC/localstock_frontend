...
export default function UserDropdown() {
  ...
  const handleSignOut = async () => {
    try {
      await dispatch(logout()).unwrap(); // Desencadena error si falla
      navigate(ROUTES.SIGNIN);
    } catch (error) {
      console.error('Error signing out:', error);
      // Aquí podrías usar un toast, modal, etc.
    }
  };
  ...
  return (
    <div className="relative">
      ...
      >
        {/*<Avatar name={`${user?.name} ${user?.surnames}`} size={40} className="mr-3 border-2 border-solid" />*/}
